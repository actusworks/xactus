...
<script>
  // The library is available as a global `Xactus` object.
  // Use `Xactus.default` to access the main factory function.
  var app = Xactus.default({
...
See [Installation](#installation) above for ESM, script tag, and bundler usage options.
...
- `bus.on(event, handler)` — subscribe to an event
- `bus.off(event, handler)` — unsubscribe (used internally on `destroy()`)

Buses that return an unsubscribe function from `.on()` (e.g. **xenonjs**, **nanoevents**) are also supported — Xactus detects both patterns automatically.
...
|---|---|
| `api.state` | Read-only snapshot of current state |
| `api.patchState(patch)` | Shallowly merge `patch` into state (does not re-render; use `setState` for that) |
| `api.setState(key, value)` | Set `state[key]` and re-render all dependent bindings |
| `api.setByPath(path, value)` | Set a nested value using `:` notation (e.g. `'user:address:city'`) |
| `api.getByPath(path)` | Get a nested value using `:` notation |
